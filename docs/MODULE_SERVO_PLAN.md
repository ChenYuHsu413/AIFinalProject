...
> 神經 autoencoder LN 0.33→HI 2.15）＋ **Phase B 真 1D-CNN**（原始波形能量包絡，留出 macro-F1 **0.729**、
> conv-AE LN 0.40→HI 0.51，見 §7）。**資料特性如實揭露**：`train_noisy_LO` 原始檔僅含 65 段
...
留出測試 **Accuracy 0.731 / macro-F1 0.729**；conv-AE 重建誤差 LN 0.40→HI 0.51 單調。windowed `.npz` 為
