...
> 神經 autoencoder 留出 LN 0.36→HI 2.20）＋ **Phase B 真 1D-CNN**（原始波形能量包絡，留出 macro-F1 **0.729**、
> conv-AE 留出 LN 0.40→HI 0.53，見 §7）。**資料特性如實揭露**：`train_noisy_LO` 原始檔僅含 65 段
...
留出測試 **Accuracy 0.731 / macro-F1 0.729**；conv-AE 留出重建誤差 LN 0.40→HI 0.53 單調。windowed `.npz` 為
gitignored 暫存（雲端只讀 JSON）。**深化方向**：更大窗 / 更多 run、原始逐點 CNN、頻譜輸入。
...
6. ✅（Phase B 完成）原始波形 1D-CNN：`build_servo_windows.py`（串流→能量包絡）+ `servo_cnn.py`（1D-CNN 分類 + conv-AE）；留出 macro-F1 0.729、後端 `/servo/cnn_results`、報表頁顯示。原始 FMCRD 用 `Downloads/FMCRD_Data.zip`（CRC 對齊溯源指紋）。
