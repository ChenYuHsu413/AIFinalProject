...
> **狀態（2026-06-25）**：主線重構第一輪完成。以 PHM Servomotor-Driven Ballscrew
> 退化資料為主線；Reference Model（健康分類 + DV 回歸）、AI 訓練模擬器、馬達欄位
> 解釋、LLM 維護助理（含離線 fallback）、維修知識庫（TF-IDF RAG）、深度學習離線
> baseline 皆已可運作。**目前以 placeholder 合成資料訓練**，待下載真實 PHM 資料後重訓。
...
`degradation_score`（DV）、`health_score`、`risk_level`、`top_features`（vs 健康基線的 z 偏離 + 白話提示）、
...
- **LLM 維護助理**：`src/llm/maintenance_assistant.py`。接收結構化輸出 + 檢索片段，生成
  ...
  需由現場人員確認），**無 `ANTHROPIC_API_KEY` 時走離線 fallback 範本**。模型預設 `claude-opus-4-8`
  （此層級不可送 temperature/top_p）。
