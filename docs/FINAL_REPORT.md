...
> **狀態（2026-06-29）**：完整書面報告。涵蓋主線 **模組 Servo** 與對照軌 **A / B / B+ / C**、
...
> §1/§2/§15 重新框定**模組與伺服馬達的相關性分級**（Servo > C 電氣模態 > B/B+ 軸承 > A 合成方法基礎），
> 淡化合成的模組 A 為方法基礎對照、新增 [`outputs/figures/servo_modality_matrix.svg`](../outputs/figures/servo_modality_matrix.svg) 對應圖。
...
由一條主線與四條對照軌組成（**依與伺服馬達的貼近程度排列**）：

- **主線 · 模組 Servo**（**專案核心 = 馬達本體**）：伺服馬達健康狀態分類（LN/LO/MED/HI）+ 退化值（DV）回歸，真實 PHM FMCRD。
- **模組 C**（**最貼近馬達本體**）：Paderborn **馬達定子電流（MCSA）** 故障診斷——PMSM 正是伺服所用馬達，補上**電氣模態**，並驗證「人工故障訓練 → 真實損傷測試」的泛化落差。
- **模組 B / B+**：NASA/IMS、XJTU-SY 軸承 run-to-failure——對應伺服的**機械失效模式（軸承劣化）** 動態健康度與 RUL，B+ 另驗多軸承 / 多工況泛化。
- **模組 A**（**通用方法基礎，最不貼近伺服**）：UCI AI4I 2020 **合成**製程風險分類——無馬達、無時間維度，僅作為預測性維護方法學的入門對照。

主線敘事為：**以真實伺服馬達退化為核心，向外以馬達電流（電氣模態，C）、軸承振動（機械失效模式，B/B+）
補齊診斷面向，並以合成製程資料（A）作為方法學基礎對照**；全程以誠實性紅線標示各資料的真實性與適用範圍。
系統另含 **AI 訓練模擬器**、**LLM 多供應商維護助理**、**TF-IDF 維修知識庫**，並以 **Next.js Command Center**
前端整合為工業監控介面。
...
> **模組與伺服馬達的相關性分級**（見圖 [`outputs/figures/servo_modality_matrix.svg`](../outputs/figures/servo_modality_matrix.svg)）：
> **Servo（核心，馬達本體）> C（馬達電流 MCSA，最貼近馬達）> B / B+（軸承，伺服的機械失效模式）> A（合成通用 PdM，最不貼近，僅方法基礎）**。
> 其中**軸承故障**可同時由**振動（B / B+）** 與**馬達電流（C）** 兩種感測模態診斷，互為補充；模組 A 為合成、無馬達、無時間維度，
> 在本報告中僅定位為預測性維護方法學的入門對照，不作為伺服馬達結果的主要依據。
...
**AI Servo Motor Health Command Center** — 亮/暗色可切換的工業風監控介面（視覺語言參考 Kiranism shadcn
dashboard、字型思源黑體、響應式）。頁面：Overview = **Command Center 戰情室**（全廠狀態 / 立即處理 / 產線地圖 / 操作導向設備卡 + 指針儀表 / 工單佇列 / AI 維護摘要）、
...
| 模組 | 任務 | 關鍵指標 | 一句話結論 |
| --- | --- | --- | --- |
| A（AI4I） | 故障二元分類 | F1 0.887 · PR-AUC 0.906 · Recall 0.809 | 合成資料上靜態風險分類可用，偏重 Recall（**方法基礎 · 最不貼近伺服**）|
...
本系統以**真實伺服馬達健康主線（Servo）** 為核心，向外以**馬達電流診斷（C，最貼近馬達本體）** 補上
電氣模態並揭露「人工→真實」泛化落差、以**軸承振動 run-to-failure（B / B+）** 涵蓋機械失效模式並誠實
驗證泛化邊界（區分可泛化的健康監測與受限的絕對 RUL），並以**合成製程資料（A）** 作為預測性維護的
方法學基礎對照，最終整合為一套可線上展示的**工業監控指揮中心**。全程以誠實性紅線標示各結果的資料
真實性與適用範圍——這正是本專案最核心的工程素養與報告防禦立場。
