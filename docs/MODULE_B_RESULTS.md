...
> **狀態（2026-06-24）**：IMS 趨勢外推 RUL 已於 2026-06-22 完成（產物見 `outputs/metrics/ims_rul.json`、`ims_rul_predictions.csv`）；**模組 B+（XJTU 多軌跡 / 多工況泛化）已完成**（15 顆 × 3 工況；健康監測 FPT 跨工況泛化成立，監督式絕對 RUL 受壽命尺度/domain shift 限制；見下方專節與 `outputs/metrics/xjtu_*`）；**延伸軌 E1（跨工況自適應 RUL）已完成**（baseline + 壽命比例 / transductive z-score / CORAL 消融，最佳 CORAL 把 LOCO R² −1.22 抬到 −0.92、oracle 上界 +0.15；`src/models/eval_xjtu_domain_adapt.py`）；**E2（維護建議決策層）已完成**（純函式 `src/models/maintenance_advice.py` + Dashboard 卡片）；**E3（即時串流回放）已完成**（B+ 頁 `st.fragment` 逐快照重播 HI/FPT/RUL，重用既有函式、無新產物）；三軌延伸 E1/E2/E3 全數完成。1D-CNN AE 雙印證已推遲、尚未實作。
...
- **Dashboard**：B+ 頁新增「維護建議（決策支援 · E2）」區，含**巡檢檢查點滑桿**（佔壽命比例）；對 15 顆軸承在該時點各畫一張風險卡片。檢查點分布隨時間推進合理（例：40% → 5🟢/10🟡/0🔴；70% → 4🟢/10🟡/1🔴；100% → 15🔴）。
...
> **狀態（2026-06-24）**：已完成（純 UI、無新產物、無新依賴）。規格見 [`MODULE_B_PLUS_EXTENSIONS_PLAN.md`](MODULE_B_PLUS_EXTENSIONS_PLAN.md) 之 E3。

把整套監測流程演成**會動的監測台**：選一顆軸承，沿時間軸**逐快照**重播。

- **回放核心**：對「前 *k* 筆」呼叫既有 `build_health_indicator` / `detect_fpt` / `extrapolate_rul`——正是系統只看過前 *k* 個快照時當下會算出的健康度、退化起點與 RUL；健康基線與失效門檻為**預先校準**的固定參考線（座標軸固定，曲線「長進」穩定畫框）。
- **控制**：▶ 播放 / ⏸ 暫停 / ⏪ 上一格 / ⏩ 下一格 / ⏮ 重置 + 速度（0.5x–4x）。以 `st.fragment` + `st.rerun(scope="fragment")` 局部重跑，動畫**不重算** E1/E2/疊圖。
- **視覺**：HI 一格格長、跨過 FPT 後目前點轉紅並標示 ★、RUL 即時外推；下方四個即時指標（進度 / 健康分數 / 即時 RUL / 退化起點）＋**重用 E2** 的即時建議卡片。驗證（Bearing1_3）：k=13 綠、k=20 過 FPT 轉黃、k=158 健康度跌破告警轉紅，RUL 收斂趨零。
...
| 維護建議卡片（E2，Dashboard） | `app/streamlit_app.py`（B+ 頁「維護建議（決策支援 · E2）」）|
| 即時串流回放（E3，純 UI） | `app/streamlit_app.py`（`_xjtu_replay` fragment）＋ `src/ui/charts.py`（`xjtu_replay_frame`）|
