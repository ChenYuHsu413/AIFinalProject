...
> **狀態（2026-06-24）**：E1/E2/E3 **全步驟完成**（見各表「狀態」欄）。

### E1 — 跨工況自適應 RUL
| 步 | 工作 | 驗收 | 狀態 |
| --- | --- | --- | --- |
| 1 | 壽命比例目標版 LOCO | 還原小時數正確；輸出 LOCO-ratio pooled / per-condition R² 對照表 | ✅ |
| 2 | 工況感知正規化 + CORAL | 兩手段各跑一輪，無 target 標籤洩漏（測試通過） | ✅（`tests/test_domain_adapt.py`） |
| 3 | 消融對照表（baseline vs 三手段） | 一張表並列 pooled R² / MAE，標示最佳手段 | ✅（最佳 CORAL −0.92） |
| 4 | 回寫 `MODULE_B_RESULTS.md` + Dashboard 顯示對照 | 文字成段、誠實標註 DA 性質與改善幅度 | ✅ |

### E2 — 維護建議決策層
| 步 | 工作 | 驗收 | 狀態 |
| --- | --- | --- | --- |
| 1 | `maintenance_advice()` 純函式 + 單元測試 | 綠/黃/紅與時間窗在合成輸入下符合預期 | ✅（9 項測試） |
| 2 | Dashboard 每軸承建議卡片 | 顯示風險等級 + 建議維護時間窗 + 理由 | ✅（「B+ 延伸應用」頁 E2 tab） |
| 3 | （選配）成本對照 | 給定成本參數標出「建議維護點」，參數可調 | ✅（「顯示成本對照」開關） |

### E3 — 即時串流回放

> **狀態（2026-07-02）**：修正回放核心的等價性描述——滾動擬合為回溯式，但**失效門檻（fail_percentile）以整段軌跡離線校準後固定**，非嚴格「只看前 k 筆」的線上計算；API docstring 與前端說明文字已同步改為此措辭。

| 步 | 工作 | 驗收 | 狀態 |
| --- | --- | --- | --- |
| 1 | 逐前綴重算 HI/FPT/RUL 的回放核心 | 畫面前進一格、數值正確 | ✅（回溯式滾動擬合；`rul[k]` 在「失效門檻已預先校準固定」前提下等價於只看前 k 筆） |
| 2 | play/pause/速度控制 + FPT 亮燈 | 連續播放至失效；FPT 在正確索引觸發 | ✅（Plotly 瀏覽器端動畫；速度鍵可即時切速、拉桿取代單格 step） |
