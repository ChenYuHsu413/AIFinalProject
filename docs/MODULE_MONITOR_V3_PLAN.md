...
- **標籤是注入的**：`fault_stage / health / rul / failure_probability` 皆為同一條 severity 斜坡
  （固定 30%/53%/73%/88% 轉階段）的確定性函數，感測欄位也是同一條 severity + 小雜訊 →
  **這就是模型 F1=1.00、提前量≈固定 horizon 的原因**。本軌價值是**即時視覺與流程**，非難度基準。
  再多資料也不會變難（產生器結構決定）。
