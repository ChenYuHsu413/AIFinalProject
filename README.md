...
系統由三條**互補但獨立**的軌道組成，將專案從「靜態風險評估」延伸到「動態健康度預測」，
再以多軌跡、多工況資料**驗證其泛化邊界**：
...
flowchart TB
    ...
    MA --> GOAL["靜態風險 → 動態健康度 → 多軌跡泛化驗證"]
    MB --> GOAL
    MBP --> GOAL
```

| 面向 | 🅰 模組 A · 靜態風險 | 🅱 模組 B · 動態健康度 | 🅱➕ 模組 B+ · 多軌跡泛化 |
| --- | --- | --- | --- |
| 資料集 | UCI **AI4I 2020**（合成） | NASA/**IMS 軸承 Set 2**（實測 run-to-failure）| **XJTU-SY**（實測，15 顆 / 3 工況）|
| 資料型態 | 單筆**點資料**（製程工況快照） | **20 kHz 高頻振動**時間序列 | **25.6 kHz 振動** × 15 條軌跡 |
| 感測量 | 溫度 / 扭矩 / 轉速 / 刀具磨耗 | 加速度振動（時域 + 頻域特徵） | 加速度振動（水平 / 垂直）|
| 目標 | 故障 / 正常**二元分類** | **健康分數退化 + 剩餘壽命（RUL）** | **跨軸承 / 跨工況泛化驗證** |
| 建模方法 | 監督式分類（10 模型 × 5 特徵組） | 健康指標 + FPT 偵測 + **指數趨勢外推** | 固定參數 FPT/外推 + **LOBO / LOCO** + 領域自適應(E1) |
| 評估指標 | F1 / Recall / ROC-AUC / PR-AUC | RUL MAE / RMSE（小時）、退化提前量 | FPT 提前量、R²（LOBO / LOCO / 自適應）|
| 主要輸出 | 故障機率、健康分數、維護建議 | 退化曲線（100→0）、RUL、告警提前量 | 多軌跡健康疊圖、泛化結論、維護建議(E2)、串流回放(E3) |
| 時間維度 | 無（靜態快照） | 有（全壽命**動態**演進） | 有（15 條獨立軌跡）|
| 對應頁面 | 單筆預測 / What-if / 批次 / 評估 | 健康度總覽 / RUL 預測 / 互動探索 | 多軌跡泛化 |
| 核心限制 | 合成資料、無 RUL 標籤 | 單一退化軌跡、突發失效 → RUL 偏粗 | 絕對 RUL 跨壽命尺度 / 工況受限（E1 部分改善）|

> 三軌**無法合併成同一個模型**（物理量、感測器、目標皆不同），因此在系統中以平行的獨立
> 軌道呈現。B+ 以多軌跡、多工況驗證「健康監測可泛化、絕對 RUL 受限」。細節見
> [`docs/MODULE_B_RESULTS.md`](docs/MODULE_B_RESULTS.md) 與
> [`docs/MODULE_B_PLUS_XJTU_PLAN.md`](docs/MODULE_B_PLUS_XJTU_PLAN.md)。
...
- **多軌跡泛化** — 15 顆 / 3 工況固定參數健康監測、健康指標疊圖、LOBO / LOCO 對照；
  含**跨工況自適應 RUL（E1）**：在 LOCO 切分上消融壽命比例 / transductive z-score / CORAL
  三種目標未標註的領域自適應，把 R² −1.22 抬到 −0.92（oracle 上界 +0.15，診斷瓶頸為壽命尺度）；
  以及**維護建議決策層（E2）**：以巡檢檢查點滑桿，把健康度 / FPT / RUL 轉成每顆軸承的
  風險等級（🟢🟡🔴）＋建議維護時間窗＋理由（決策支援，非控制）；
  還有**即時串流回放（E3）**：選一顆軸承逐快照重播 HI / FPT / RUL（play/step 控制，離線資料重播）。
...
> **狀態（2026-06-24）**：模組 B+ 延伸 E1（跨工況自適應 RUL）/ E2（維護建議決策層）/ E3（即時串流回放）已完成；下方「未來工作」已據此更新。
...
### 已完成的 B+ 延伸（見 [`docs/MODULE_B_PLUS_EXTENSIONS_PLAN.md`](docs/MODULE_B_PLUS_EXTENSIONS_PLAN.md)）
- **E1 跨工況自適應 RUL**：在 LOCO 切分上消融三種目標未標註的領域自適應，把 R² 由 −1.22 抬到 −0.92。
- **E2 維護建議決策層**：健康度 / FPT / RUL → 風險等級 + 建議維護時間窗 + 理由（決策支援）。
- **E3 即時串流回放**：逐快照重播 HI / FPT / RUL 的「會動的監測台」（離線資料重播）。
