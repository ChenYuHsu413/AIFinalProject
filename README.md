...
**🛰 即時監控雷達（合成 demo · 獨立軌）**
> **狀態（2026-07-03）**：新增獨立合成 demo 軌（Next.js `/monitor`），含**即時感測器串流**與**情境回放**兩模式。
> **即時模式**：後端 SSE（`/monitor/stream`）以 **vendored 產生器即時生成 + 即時推論**，隨機注入 30 種故障，
> 事件串流呈現「注入 → 模型預警 → 告警」（提前 ~5s）。**回放模式**：30 情境逐幀播放 + 六角雷達 + 演進時間軸。
> 早期預警（留出情境）F1 1.00、中位提前量 5.0s。**一切由 committed 產生器重現、不需原始大檔；合成資料、
> 模擬感測器、與真實 PHM 主線分開、不取代**。細節見 [`docs/MODULE_MONITOR_V3_PLAN.md`](docs/MODULE_MONITOR_V3_PLAN.md)。
- **即時監控雷達** — 即時 ⇄ 回放切換；六角雷達 + 健康環 + RUL + 告警燈 + 事件串流 / 演進時間軸。
