...
def _few_shot_curve(train_df: pd.DataFrame, real_df: pd.DataFrame, feats: List[str],
                    model_name: str, ks: List[int], seeds: int, rs: int) -> List[Dict]:
    ...
    for k in ks:
        ...
        for s in range(seeds):
            ...
            tr = pd.concat([train_df, shots], ignore_index=True)
            pipe = _make_pipe(model_name, feats, rs).fit(tr[feats], tr["fault_class"])
