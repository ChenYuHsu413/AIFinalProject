...
> 下方大綱保留為 A/B/B+ 的原始骨架。
>
> **相關性分級（2026-06-29）**：A/B/B+/C 皆為對照軌；依與伺服馬達的貼近程度為
> **Servo（核心）> C（馬達電流 MCSA，最貼近馬達）> B/B+（軸承，伺服的機械失效模式）> A（合成通用 PdM，最不貼近，僅方法基礎）**。
> 模組 A 為合成、無馬達、無時間維度，已**淡化**為方法學入門對照，非伺服結果的主要依據——詳見 FINAL_REPORT §1–2、§15。
...
2. （推遲）其餘公開資料集（FEMTO / Mendeley / PMSM）、RUL 壽命正規化 / 領域自適應、
   ...
   （**模組 B（IMS）1D-CNN Autoencoder 深度對照已確定不做（2026-06-29）** —— DL 已由 Servo 主線 Phase A/B 達成。）
